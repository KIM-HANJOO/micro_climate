...
num_all = len(os.listdir(sample_avail))

for num_excel, excel in enumerate(os.listdir(sample_avail)) :
    ...
    temp_tran = temp[transmission_time]
    temp_regi = temp[register_date]


    for index in range(temp.shape[0]) :
        if int(temp_tran.loc[index, :]) == int(temp_regi.loc[index, :]) :
            same_num += 1
        else :
            diff_num += 1

    print(f'{num_excel + 1} / {num_all}')
