...
def plot_location(location, plot_dir, save_name, title) :
...
#os.listdir(os.path.join(sample_robby, 'only_available_time'))

#os.chdir(sample_plot)
print(os.listdir(sample_plot))
info_32 = read_excel('samples_32.xlsx')

drop_index = []
for index in range(info_32.shape[0]) :
    check = 0
    for item in available_samples :
        if item in  info_32.loc[index,'시리얼번호'] :
            check = 1

    if check == 0 :
        drop_index.append(index)
info_32.drop(drop_index, inplace = True)
info_32.reset_index(drop = True, inplace = True)
...
plot_location(info_32, sample_plot, 'available_time', 'used stations')
