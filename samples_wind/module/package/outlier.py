...
def z_score(list1, coe) :
    # standard cut-off for outlier is coe == 3
    zscore = scipy.stats.zscore(list1)

    non_out = []
    outliers = []
    for num_item, item in enumerate(list1) :
        if (zscore[num_item] > coe) | (zscore[num_item] < -1 * coe) :
            outliers.append(item)
        else :
            non_out.append(item)


    return non_out, outliers


def out_box(list1, whis) :
    # standard cut-off for outliers is whis == 1.5
    non_out = []
    outliers = []
    ...
    for item in list1 :
        if (item > Q1 - whis * IQR) & (item < Q3 + whis * IQR) :
            non_out.append(item)
        else :
            outliers.append(item)
