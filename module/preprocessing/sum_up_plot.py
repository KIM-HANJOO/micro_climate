...
# check minutes, null values for all
...
columns_rename = ['기관명', '모델명', '시리얼', '구분', '초미세먼지', '미세먼지', '기온', '상대습도', '풍향', '풍속', '돌풍 풍향', '돌풍 풍속', '조도', '자외선', '소음', '진동_x', '진동_y', '진동_z', '진동_x_최대', '진동_y_최대', '진동_z_최대', '흑구 온도', '초미세먼지 보정', '미세먼지 보정', '전송시간', '등록일자']


def target_columns(column_list) :
    except_list = ['기관 명', '모델명', '시리얼', '구분', '등록일자', '소음', '흑구', '일산화탄소', '이산화질소', '이산화황', '암모니아', '황화수소', '오존', '기관명', '등록 일자']
    ...
    for col in column_list :
        check = 0
        for except_col in except_list :
            if except_col in col :
                check = 1

        if check == 0 :
            columns_watch.append(col)

    return columns_watch
...
for excel_num, excel in enumerate(os.listdir(merged_dir)) :
    if 'V' in excel :
        ...
        break
#
#all_station_numbers = len(os.listdir(serials_dir))
#for excel_num, excel in enumerate(os.listdir(merged_dir)) :
#    if ((excel_num > 2) & (excel_num < excel_range + 3)) :
#        os.chdir(merged_dir)
#        temp = read_excel(excel)
#
#        temp = temp[target_columns(temp.columns)]
#
#        all_index = temp.shape[0]
#        null_count = temp['전송시간'].isna().sum()
##        if excel_num == 0 :
##            info_count.columns = temp.columns
#
#        for col in temp.columns :
#            if '전송시간' not in col :
#                if col not in info_count.columns :
#                    info_count.loc[0, col] = 0
#                    info_count.loc[1, col] = 0
#                    info_count.loc[2, col] = 0
#
#                all_values = temp.shape[0]
#                null_values = temp.loc[:, col].isnull().sum()
#                non_null = all_values - null_values
#
#                info_count.loc[0, col] += all_values
#                
#                if non_null == 0 :
#                    info_count.loc[1, col] += 1
#
#                info_count.loc[excel_num + 2, col] = non_null
#        print(excel_num, '\t', excel)
#
#os.chdir(plot_dir)
#info_count.to_excel(f'info_count_O_{excel_range}.xlsx')
#print(info_count)
#
#                
...
all_station_numbers = len(os.listdir(serials_dir))
for excel_num, excel in enumerate(os.listdir(merged_dir)) :
    if ((excel_num > 250 + 100) & (excel_num < excel_range + 251 + 100)) :
        os.chdir(merged_dir)
        temp = read_excel(excel)
...
#        new_columns = []
#        for col_num, col in enumerate(temp.columns) :
#            if col_num < 19 :
...
#                to_col = col.replace('*', '')
#
#                temp.loc[:, to_col] = temp.loc[:, to_col].replace(',','').astype(float) + temp.loc[:, col].replace(',','').astype(float)
##                temp[to_col] = pd.Series(temp[to_col] + temp[col])
#
#                temp.drop([col], axis = 1, inplace = True)
#

#        for col_num, col in temp.columns :
#            if isinstance(temp[col].isnull().sum(), np.int64) :
#                pass
#            else :
#                temp[f'{col}_com'] = temp[col].iloc[:, 0] + temp[col].iloc[:, 1]
#                temp.drop([col], axis = 1, inplace = True)
            ...
            if col_num < 19 :
                if '전송시간' not in col :
                    if col not in info_count.columns :
                        ...
                        info_count.loc[1, col] = 0
                        info_count.loc[2, col] = 0
                        print(f'wrongwrong {excel}')

                    all_values = temp.shape[0]
                    null_values = temp.loc[:, col].isnull().sum()
                    non_null = all_values - null_values
                    print(len(info_count.columns))
...
os.chdir(plot_dir)
info_count.to_excel(f'info_count_V_{excel_range}.xlsx')
print(info_count)

        
#        null_perc = round(null_count / all_index * 100, 2)
#        print(f'{null_count}/{all_index}')
#        print(null_perc)
#        
#        ave_null_perc.append(null_perc)
#
#        print(f'{excel_num} / {all_station_numbers}\t{null_count}/{all_index}')
#
#ax1.boxplot(ave_null_perc)
#ax1.set_title('percentage of null values for all statinos')
#
#
#dlt.savefig(plot_dir, 'ave_null_perc.png', 400)
