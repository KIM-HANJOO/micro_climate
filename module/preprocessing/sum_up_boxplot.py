...
os.chdir(plot_dir)
info_o = read_excel('info_count_O_100.xlsx')
info_v = read_excel('info_count_V_100.xlsx')

info_o.reset_index(drop = True, inplace = True)
info_v.reset_index(drop = True, inplace = True)


# info_o
fig = plt.figure(figsize = (10, 7))

for col_num, col in enumerate(info_o.columns) :
    plt.boxplot(info_o.loc[3 :, col], positions = [col_num + 1])

xlabel_set = info_o.columns.tolist()
xlabel_df = list(range(1, len(xlabel_set) + 1))

print(xlabel_df)
print(xlabel_set)
plt.plot([min(xlabel_df) - 2, max(xlabel_df) + 2], [9576, 9576], c = 'r', label = '9576 hours')
plt.ylabel('hours available')
plt.xlim(min(xlabel_df) - 1, max(xlabel_df) + 1)
plt.legend()

plt.subplots_adjust(top = 0.9, bottom = 0.23)
plt.xticks(xlabel_df, xlabel_set, rotation = 90)
plt.title('OC3CL serial numbered stations\n100 samples')
dlt.savefig(plot_dir, 'o_serial.png', 400)

    
# info_v
fig = plt.figure(figsize = (10, 7))

for col_num, col in enumerate(info_v.columns) :
    plt.boxplot(info_v.loc[3 :, col], positions = [col_num])

xlabel_set = info_v.columns
xlabel_df = list(range(len(xlabel_set)))

print(xlabel_df)
print(xlabel_set)
plt.plot([min(xlabel_df) - 2, max(xlabel_df) + 2], [16415, 16415], c = 'r', label = '16415 hours')
plt.ylabel('hours available')
plt.xlim(min(xlabel_df) - 1, max(xlabel_df) + 1)
plt.legend()
...
plt.subplots_adjust(top = 0.9, bottom = 0.23)
plt.xticks(xlabel_df, xlabel_set, rotation = 90)
plt.title('V02Q1 serial numbered stations\n100 samples')
dlt.savefig(plot_dir, 'v_serial.png', 400)
