...
# sum up the excels in each directories(station representatives)
...
#print(os.listdir(serials_dir))

# 1451 number
for station_number, station in enumerate(os.listdir(serials_dir)) :
    if len(station) > 7 :
        if station_number > 1451 :
            station_dir = os.path.join(serials_dir, station)

            for num_excel,excel in enumerate(os.listdir(station_dir)) :
            ...
            print(station)
            print(df.shape[0])
            df.to_excel(f'{station}.xlsx')
            print(f'{station_number} / {all_station_numbers}, perc = {round(station_number / all_station_numbers, 2) * 100}%')
