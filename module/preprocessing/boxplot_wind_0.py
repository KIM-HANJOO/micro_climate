...
#            tg_nan = tg.loc[:, 'speed'].dropna()
##            tg_nan = [x for x in tg.loc[:, 'speed'].tolist() if str(x) != 'NaN']
#            tg_10 = [x for x in tg_nan if int(x) < 10]
#            tg_all += tg_10
#
#
#
#            for index in range(tg.shape[0]) :
#                if tg.loc[index, 'speed'] == 0 :
#                    if tg.loc[index, 'angle'] == 0 :
#                        add_num += 1
#
#
...
#
#plt.hist(tg_all)
#plt.plot([0, 1], [add_num, add_num], c = 'r')
#plt.xlim(0, 7)
#
#plt.ylabel('index(hours)')
#plt.xlabel('speed(m/s)')
#plt.title('wind_boxplot_speed')
#
#
#dlt.savefig(plot_dir, 'wind_boxplot_zeros.png', 400)
#
