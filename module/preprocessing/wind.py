...
number_limit = 20
number = 0
all_num = len([x for x in os.listdir(merged_dir) if 'V02Q1' in x])

angle_dir = os.path.join(plot_dir, 'angle')
all_size = 0
...
for excel_num, excel in enumerate(os.listdir(merged_dir)) :
    if 'V02Q1' in excel :
        while number < number_limit :
            ...
            tg = temp[wind_col]
            tg.columns = ['angle', 'speed']

            size = tg.shape[0]

            tg.sort_values(by = ['angle'], inplace = True)
            tg.reset_index(drop = True, inplace = True)

            all_angles = tg['angle'].unique().tolist()
            
            for index in range(tg.shape[0]) :
                if tg.loc[index, 'angle'] < 361 :
                    if tg.loc[index, 'speed'] < 50 :
                        plt.scatter(tg.loc[index, 'angle'], tg.loc[index, 'speed'])
...
plt.title(f'{number_limit} of stations\n{all_size} of index, hist of wind speed')

dlt.savefig(plot_dir, 'wind_boxplot.png', 400)
