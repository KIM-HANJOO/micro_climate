...
#info_clu, only_centers = cluster_new(location.loc[:, '경도' : '위도'], 10)
...
for k in range(K) :
    if k != -1 :
        ...
        for num, index in enumerate(range(cluster_now.shape[0])) :
            dt = distance(cluster_now.loc[index, '경도' : '위도'].tolist(), center)
            ...
            if num == 0 :
                ...
                if float(dt) < float(shortest) :
                    ...
                    short_index = index
        print(k)
        df_sample.loc[0, k] = cluster_now.loc[short_index, '시리얼번호']
