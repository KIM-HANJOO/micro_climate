...
if a == 'y' :
    ...
    for target_info in target_info_list :
        ...
        for num_excel, excel in enumerate(os.listdir(sample_only_time)) :
            ...
            if target_info in temp.columns :

                if temp.shape[0] <= 1 :
                    print(f'{num_excel} shape : {temp.shape[0]}')
                ...
                print(f'{target_info} not in {excel}')

            print(f'{num_excel} / {num_all}')

        temp_dir = os.path.join(sample_plot, 'null_outliers_check')
