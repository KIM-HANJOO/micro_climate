...
merge_columns['초미세먼지'] = ['초미세먼지(㎍/㎥)', '초미세먼지 (㎍/㎥)', '초미세먼지(㎍/㎥)']
merge_columns['미세먼지'] = ['미세먼지(㎍/㎥)', '미세먼지 (㎍/㎥)']
merge_columns['초미세먼지보정'] = ['초미세먼지 보정(㎍/㎥)', '초미세먼지 보정 (㎍/㎥)', '초미세먼지보정(㎍/㎥)']
merge_columns['미세먼지보정'] = ['미세먼지 보정(㎍/㎥)', '미세먼지 보정 (㎍/㎥)', '미세먼지보정(㎍/㎥)']
merge_columns['조도'] = ['조도(lux)', '조도(lux)']
...
print(f'save_target = {save_target}')
...
if a == 'y' :
    ...
    for num_excel, excel in enumerate(os.listdir(sample_data)) :
        ...
        skip = 0
        if temp.shape[0] < 2 :
            print(f'{temp.shape}, {excel} is dropped\t{num_excel}/{all_excel}')
            skip = 1
        ...
        if skip != 1 :
            ...
            for col in temp.columns : 
                ...
                if check == 0:
                    print(f'{col} is not available(no data)')
            ...
            for col in temp.columns :
                ...
                for key in save_columns : 
                    ...
                    elif col.replace(' ', '') in merge_columns[key] :
                        check = 1
                        break

                    elif col.replace(' ', '') in save_target :
                        check = 1
                        break
            ...
            for col in temp.columns :
                if col in save :
                    ...
                    if check == 0 :
                        ...
                        print(f'{col} will be changed to {column_change}')
                        if column_change not in temp.columns :
                            temp[column_change] = None
            ...
            print(f'length : {len(temp.columns)}')
            temp.to_excel(excel)
